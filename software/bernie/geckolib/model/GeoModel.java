...
public abstract class GeoModel<T extends GeoAnimatable> {
		...
		if (model == null) {
			...
			if (!location.equals(strippedPath)) {
				GeckoLibConstants.LOGGER.debug("Unnecessary prefix or suffix found in model resource path: {} ({}). Remove this from your getModelResource",
											   location.method_12832(), location.method_12832().replace(strippedPath.method_12832(), ""));
	...
	public Animation getAnimation(T animatable, String name) throws RuntimeException {
		...
		class_2960[] fallbackLocations = getAnimationResourceFallbacks(animatable);
	...
	public void prepareForRenderPass(T animatable, GeoRenderState renderState) {
		...
		class_10017 entityRenderState = renderState instanceof class_10017 state ? state : null;
		double animatableRenderTime = entityRenderState != null ? entityRenderState.field_53328 : renderState.getGeckolibData(DataTickets.TICK);
