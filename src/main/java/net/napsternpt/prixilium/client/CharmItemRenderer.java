...
public class CharmItemRenderer extends GeoItemRenderer<CharmItem> {
    private static final Identifier FLAT_TEXTURE = Identifier.of(Prixilium.MOD_ID, "textures/item/charm_i_2d.png");
    ...
    public void render(GeoRenderState renderState, MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
        ...
        if (perspective == ItemDisplayContext.GUI) {
            ...
            renderFlat2D(matrices, vertexConsumers);
            return;
        }
        if (perspective == ItemDisplayContext.FIXED) {
            renderFlat2D(matrices, vertexConsumers);
            return;
        }
        super.render(renderState, matrices, vertexConsumers);
    }
    private void renderFlat2D(MatrixStack matrices, VertexConsumerProvider vertexConsumers) {
        matrices.push();
        matrices.translate(0.5f, 0.5f, 0.5f);
        RenderLayer renderLayer = RenderLayer.getEntityCutoutNoCull(FLAT_TEXTURE);
